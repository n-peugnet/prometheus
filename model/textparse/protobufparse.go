...
type ProtobufParser struct {
	...
	// Used for both the string returned by Series, as well as,
...
func (p *ProtobufParser) Series() ([]byte, *int64, float64) {
	...
	if *ts != 0 {
		return p.seriesBytes.Bytes(), ts, v
	}
	...
	return p.seriesBytes.Bytes(), nil, v
}
...
func (p *ProtobufParser) Histogram() ([]byte, *int64, *histogram.Histogram, *histogram.FloatHistogram) {
	...
	if h.GetSampleCountFloat() > 0 || h.GetZeroCountFloat() > 0 {
		...
		fh.Compact(0)
		if *ts != 0 {
			return p.seriesBytes.Bytes(), ts, nil, &fh
		}
		...
		return p.seriesBytes.Bytes(), nil, nil, &fh
	}
	...
	sh.Compact(0)
	if *ts != 0 {
		return p.seriesBytes.Bytes(), ts, &sh, nil
	}
	return p.seriesBytes.Bytes(), nil, &sh, nil
}
...
func (p *ProtobufParser) Unit() ([]byte, []byte) {
	return p.seriesBytes.Bytes(), []byte(p.dec.GetUnit())
}
...
func (p *ProtobufParser) Metric(l *labels.Labels) string {
	*l = p.lset.Copy()
	return yoloString(p.seriesBytes.Bytes())
}
...
func (p *ProtobufParser) Next() (Entry, error) {
	...
	case EntryInvalid:
		...
		// into seriesBytes and validate only name, help, and type for now.
		name := p.dec.GetName()
		...
		p.seriesBytes.Reset()
		p.seriesBytes.WriteString(name)
		p.state = EntryHelp
	case EntryHelp:
		if p.dec.Unit != "" {
			p.state = EntryUnit
	...
	case EntrySeries:
		...
		if t == dto.MetricType_SUMMARY ||
			...
			// TODO(bwplotka): Add test case for the previous regression here
			// when we forgot parenthesis over the OR.
			// https://github.com/prometheus/prometheus/pull/15731/files/308d6301f3a4adf38164bd629621155c32183b4f#diff-6514f82c033f3a6d3ce48dae99e090260c0798c49a6e6c895ebbf96a583dd386
			if (t == dto.MetricType_HISTOGRAM || t == dto.MetricType_GAUGE_HISTOGRAM) &&
				isNativeHistogram(p.dec.GetHistogram()) {
				p.state = EntryHistogram
			}
...
func (p *ProtobufParser) onSeriesOrHistogramUpdate() error {
	...
	// seriesBytes has to be unique for each series.
	p.seriesBytes.Reset()
	p.lset.Range(func(l labels.Label) {
		if l.Name == labels.MetricName {
			p.seriesBytes.WriteString(l.Value)
			return
		}
		p.seriesBytes.WriteByte(model.SeparatorByte)
		p.seriesBytes.WriteString(l.Name)
		p.seriesBytes.WriteByte(model.SeparatorByte)
		p.seriesBytes.WriteString(l.Value)
	})
	return nil
